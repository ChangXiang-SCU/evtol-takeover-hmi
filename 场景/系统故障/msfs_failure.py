...
from msfs_devkit_http import DevKitError, api_get, api_get_path, api_put_path, api_set, teleport_aircraft
...
def cmd_save_heading(_: argparse.Namespace) -> None:
    ...
    point["saved_at_utc"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    point["position"]["heading_true_deg"] = hdg
    point["position"]["pitch_deg"] = pitch
    point["position"]["bank_deg"] = bank
    point["teleport_set_payload"]["PLANE_HEADING_DEGREES_TRUE"] = hdg
    point["teleport_set_payload"]["PLANE_PITCH_DEGREES"] = pitch
    point["teleport_set_payload"]["PLANE_BANK_DEGREES"] = bank
    save_archive(archive)

    print(f"系统故障起始点航向已更新: {ARCHIVE_PATH}")
