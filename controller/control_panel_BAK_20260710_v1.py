...
def _throttle_route_loop():
    """AP开=纯操纵阶梯进近:油门:1(前后)+升降舵(升降),平飞一段→下降一段→交替,
    到停机坪上方约40ft 停。近坪的下降段收油门(不再前冲)。可被 AUTO_STOP 立即打断。"""
    hp = HELIPAD
    if not hp:
        return
    ...
    def hd():
        ...
        d = geo.distance_m(la, lo, hp["lat"], hp["lng"]) if la is not None else 9e9
        return st, d

    dmin = 9e9
    while not AUTO_STOP.is_set():
        st, dist = hd(); alt = g(st, "PLANE_ALTITUDE")
        dmin = min(dmin, dist)
        near = (dist < ARRIVE) or (dist > dmin + 20)      # 到坪 或 已过最近点(veer/overshoot)
        if near and alt <= HOVER_ALT + 8:
            break
        # 平飞段:还没水平到位且没飞过头
        if dist > ARRIVE and dist <= dmin + 20:
            t = time.time()
            while not AUTO_STOP.is_set() and time.time() - t < 5.0:
                client.set_param(THR, FWD); client.set_param(ELEV, 0.0)
                _, dnow = hd(); dmin = min(dmin, dnow)
                if dnow < ARRIVE or dnow > dmin + 20:
                    break
                if AUTO_STOP.wait(1.0 / HZ):
                    break
        if AUTO_STOP.is_set():
            break
        # 下降段:还高才降;近坪则收油门只降不前冲
        st, dist = hd(); alt = g(st, "PLANE_ALTITUDE")
        if alt > HOVER_ALT + 8:
            ...
            while not AUTO_STOP.is_set() and time.time() - t < 5.0:
                ...
                if g(st2, "PLANE_ALTITUDE") <= HOVER_ALT:
                    break
                if AUTO_STOP.wait(1.0 / HZ):
                    break
        elif near:
